...
from KG.common.utils import run_random_walk
from allennlp.common.params import Params


class ConceptNetKG(KG):
    def __init__(
        self,
        nodes: list,
        ...
        """ConceptnetKG class with relations.

        Args:
            nodes (list): list of node names
            features (torch.Tensor): the initial node features
            edges (list): list of edges (u, r, v)
            relations (list): list of relation names
            params (Params, optional): Contains information
                related to the random-walk. Defaults to None.
        """
        super().__init__(
            nodes, features, edges, relations=relations, params=params
        )
    ...
    def setup_graph(self, bidirectional=False):
        ...
        Args:
            bidirectional (bool, optional): makes the edges with
            the relations as bidirectional. Defaults to False.
        """

        if self.adj_lists is None:
            edges_df = pd.DataFrame(
                self.edges, columns=["start_id", "relation_id", "end_id"]
            )
            edges_df.drop_duplicates()

            if bidirectional or self.bidirectional:
                opp_edges = edges_df.copy()
                opp_edges[["start_id", "relation_id", "end_id"]] = edges_df[
                    ["end_id", "relation_id", "start_id"]
                ]
