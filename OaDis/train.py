...
def validate_ge(epoch, model, testloader, evaluator, device, topk=1):
    ...
    return stats['AUC'], stats['best_hm'], stats['AUC_attr'], stats['hm_open_attr_all_bias_max']
...
def main_worker(gpu, cfg):
    ...
    trainset = CompositionDataset(
        phase='train', split=cfg.DATASET.splitname, cfg=cfg,open_world=cfg.TRAIN.open_world)
    ...
    valset = CompositionDataset(
        phase='val', split=cfg.DATASET.splitname, cfg=cfg,open_world=cfg.TRAIN.open_world)
    valloader = torch.utils.data.DataLoader(
        valset, batch_size=cfg.TRAIN.test_batch_size, shuffle=False,
        num_workers=cfg.TRAIN.num_workers)
    testset = CompositionDataset(
        phase='test', split=cfg.DATASET.splitname, cfg=cfg,open_world=cfg.TRAIN.open_world)
    ...
    best_records = {
        ...
        'val/best_auc_attr': 0.0,
        'val/best_hm_attr': 0.0,
        'test/auc_at_best_val_attr': 0.0,
        'test/hm_at_best_val_attr': 0.0,
    ...
    while epoch <= cfg.TRAIN.max_epoch:
        ...
        if epoch % cfg.TRAIN.eval_every_epoch == 0:
            ...
            auc, best_hm,auc_attr,best_hm_attr = validate_ge(epoch, model, valloader, evaluator_val_ge, device, topk=cfg.EVAL.topk)
            logger.add_scalar('val/auc', auc, epoch * len(trainloader))
            logger.add_scalar('val/best_hm', best_hm, epoch * len(trainloader))

            print( auc, best_hm,auc_attr,best_hm_attr)
            print(best_records)
            ...
            if not cfg.TRAIN.open_world and auc_attr > best_records['val/best_auc_attr']:
            #if auc > best_records['val/best_auc']:
                best_records['val/best_auc'] = auc
                best_records['val/best_hm'] = best_hm
                best_records['val/best_auc_attr'] = auc_attr
                best_records['val/best_hm_attr'] = best_hm_attr
                print('Beat best Val AUC, now evaluate on test set')
                # Test.
                auc, best_hm,auc_attr,best_hm_attr = validate_ge(epoch, model, testloader, evaluator_test_ge, device,topk=cfg.EVAL.topk)
                ...
                best_records['test/auc_at_best_val_attr'] = auc_attr
                best_records['test/hm_at_best_val_attr'] = best_hm_attr
                save_checkpoint(model, f'auc_attr_model_epoch{epoch}', cfg)
            ...
            if  cfg.TRAIN.open_world and best_hm_attr > best_records['val/best_hm_attr']:
                   # print(12312321)
            #if auc > best_records['val/best_auc']:
                best_records['val/best_auc'] = auc
                best_records['val/best_hm'] = best_hm
                ...
                best_records['test/auc_at_best_val_attr'] = auc_attr
                best_records['test/hm_at_best_val_attr'] = best_hm_attr
                save_checkpoint(model, f'hm_open_model_epoch{epoch}', cfg)
    ...
    print('New Best AUC attr:',best_records['val/best_auc_attr'])
    print('New Best HM attr :',best_records['val/best_hm_attr'])
