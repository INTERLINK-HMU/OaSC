...
class Evaluator:
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk = 1):
       ...
       # print(unseen_matches)
        ...
        print(1,len(unseen_ind))
        print(2,len(seen_ind))
      ...
      #  print(len(seen_attr),len(un_seen_attr),len(seen_attr)+len(un_seen_attr))
        ...
        print(seen_atr_acc,unseen_atr_acc, stats['hm_open_attr_all_bias_max']  ,area_aatr)
    ...
    def save_unseen_seen_acc(self, name):
        if not os.path.exists(name):
            os.makedirs(name)
        np.savez(os.path.join(name, 'curve.npz'), seen = self.seen_accuracy, unseen = self.unseen_accuracy)