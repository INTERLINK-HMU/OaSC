...
def main():
    ...
    if(args.attr):
        args.load = ospj(logpath,'ckpt_hm_closed_attr_max.t7')
        if(args.open_world):
            args.load = ospj(logpath,'ckpt_hm_open_attr_max.t7')
    ...
    print("Final results following for test set for split %s"%args.name)
    with torch.no_grad():
        test(image_extractor, model, testloader, evaluator, args,print_results=True)
        # evaluator.save_unseen_seen_acc('./curves/' + args.name)


def test(image_extractor, model, testloader, evaluator,  args, print_results=False):
        if image_extractor:
            image_extractor.eval()
