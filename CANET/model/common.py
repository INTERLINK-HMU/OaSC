...
class MLP(nn.Module):
    ...
    Inputs
        ...
        bias: Bool, Use bias
    '''
    def __init__(self, inp_dim, out_dim, num_layers = 1, relu = True, bias = True, dropout = False, norm = False, layers=[4096]):
...
class Evaluator:
    ...
    def add_bias_to_score(self, scores, bias = 0.0):
        mask = self.seen_mask.repeat(scores.shape[0],1) # Repeat mask along pairs dimension
        scores[~mask] += bias # Add bias to test pairs

        return scores

    def evaluate_predictions2(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk = 1):
        # Go to CPU
        ...
        closed_unseen_match = stats['closed_unseen_match']
        ...
        stats['closed_unseen_match_orig'] = closed_unseen_match
        ...
        stats['unseen_ind'] = unseen_ind
        stats['closed_logits'] = predictions['closed_logits']
        return stats
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk = 1):
      ...
      #  print(len(seen_attr),len(un_seen_attr),len(seen_attr)+len(un_seen_attr))
        ...
        stats['unseen_open_attr'] = float(unseen_atr_acc)   
