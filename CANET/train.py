...
best_open_ub_attr_match = 0
best_closed_ub_attr_match = 0

best_open_ub_attr_match_max = 0
best_closed_ub_attr_match_max = 0
best_auc_attr = 0
...
def main():
    ...
    #args.dataset = 'cgqa_states' # Choose from ut-zap50k | mit-states | cgqa
   ...
   # logpath = ospj(args.main_root, 'logs', args.dataset)
    logpath = ospj(args.main_root, 'logs', args.name)
    ...
    trainset = dset.CompositionDataset(
        args=args,
        root=ospj(args.data_root, args.data_dir),
        phase='train',
        split=args.splitname,
        model =args.image_extractor,
        update_image_features = args.update_image_features,
        train_only= args.train_only, data_dir= args.data_dir, open_world=args.open_world
)
    testset = dset.CompositionDataset(
        ...
        split=args.splitname,
        model =args.image_extractor,
        update_image_features = args.update_image_features,  data_dir= args.data_dir, open_world=args.open_world
    )
...
def test(args, epoch, image_extractor, model, testloader, evaluator, logpath):
    ...
    def save_checkpoint(filename):
       ...
       # torch.save(state, ospj(args.main_root, 'logs', args.dataset, args.name,'ckpt_{}_{}.t7'.format(filename, args.dataset)))
        torch.save(state, ospj(args.main_root, 'logs',args.name,'ckpt_{}_{}.t7'.format(filename, args.dataset)))
    ...
    global best_attr, best_obj, best_seen, best_unseen, best_auc, best_hm, best_epoch, best_open_ub_attr_match, best_closed_ub_attr_match, best_open_ub_attr_match_max, best_closed_ub_attr_match_max,best_auc_attr
    ...
    if('hm_open_attr' in stats):
            if stats['hm_open_attr'] > best_open_ub_attr_match:
                best_open_ub_attr_match = stats['hm_open_attr']
                print('New best open_ub_attr_match ', best_open_ub_attr_match)
                save_checkpoint('hm_open_attr')


    if('hm_closed_attr' in stats):
    
        if stats['hm_closed_attr'] > best_closed_ub_attr_match:
            best_closed_ub_attr_match = stats['hm_closed_attr']
            print('New best_closed_ub_attr_match  ', best_closed_ub_attr_match)
            save_checkpoint('hm_closed_attr')

    if('hm_open_attr_all_bias_max' in stats):
        if stats['hm_open_attr_all_bias_max'] > best_open_ub_attr_match_max:
            best_open_ub_attr_match_max = stats['hm_open_attr_all_bias_max']
            print('New best hm _open max ', best_open_ub_attr_match_max)
            save_checkpoint('hm_open_attr_max')


    if('hm_closed_attr_all_bias_max' in stats):
        print('hm_closed_attr_max')
        if stats['hm_closed_attr_all_bias_max'] > best_closed_ub_attr_match_max:
            best_closed_ub_attr_match_max = stats['hm_closed_attr_all_bias_max']
            print('New best hm closed max  ', best_closed_ub_attr_match_max)
            save_checkpoint('hm_closed_attr_max')

    if('AUC_attr' in stats):
        if stats['AUC_attr'] > best_auc_attr:
            best_auc_attr = stats['AUC_attr']
            print('New best_auc_attr  ', best_auc_attr)
            save_checkpoint('AUC_attr')    
