...
def main():
   ...
   # print(model)
    ...
    logpath = './logs/'+args.name
    if(args.attr):
        args.load = ospj(logpath,'ckpt_hm_closed_attr_max_%s.t7'%args.dataset)
       
        if(args.open_world):
             args.load = ospj(logpath,'ckpt_hm_open_attr_max_%s.t7'%args.dataset)
        
    if(args.hm):
        args.load = ospj(logpath,'ckpt_best_hm_%s.t7'%args.dataset)
     
     
    if(args.auc):
        args.load = ospj(logpath,'ckpt_best_auc_%s.t7'%args.dataset)
...
def test(image_extractor, model, testloader, evaluator, args):
    ...
    HM_attr=stats['hm_open_attr_all_bias_max']
    seen_ac_attr=stats['seen_open_attr_all_bias_max']
    unseen_ac_attr=stats['unseen_open_attr_all_bias_max']
    print("%s"%args.name)
    print('|----Test Finished: Attr Acc: {:.2f}% | Obj Acc: {:.2f}% | Seen Acc: {:.2f}% | Unseen Acc: {:.2f}%  | HM: {:.2f}% | AUC: {:.2f}%.\
| Seen Acc attr: {:.2f}% | Unseen Acc attr: {:.2f} | HM_attr: {:.2f}% | AUC_attr: {:.2f}%'.\
    format(attr_acc*100, obj_acc*100, seen_acc*100, unseen_acc*100, HM*100, AUC*100,seen_ac_attr*100, unseen_ac_attr*100,HM_attr*100, AUC_attr*100))
