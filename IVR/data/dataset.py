...
class CompositionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'train':
            positive_attr = self.same_A_diff_B(label_A=attr, label_B=obj, phase='attr',image_path=image)
            same_attr_image = positive_attr[0]
            one_obj=positive_attr[2]
            one_attr = positive_attr[1]
          
            positive_obj = self.same_A_diff_B(label_A=obj, label_B=attr, phase='obj',image_path=image)
    ...
    def same_A_diff_B(self, label_A, label_B, phase='attr',image_path=None):
        data1 = []
        
       # print(len(self.train_data),len(self.attrs),len(self.objs))
 
        if(len(self.attrs)==1 or len(self.objs) ==1):
                if phase=='attr':    
                    return [image_path,label_A, label_B,]
                else:
                    
                    return [image_path,label_B, label_A,]
